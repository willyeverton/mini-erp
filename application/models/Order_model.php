...
class Order_model extends CI_Model {
    ...
    public function get_order_items($order_id) {
        $this->db->select('order_items.*, products.name as product_name, variations.name as variation_name');
        $this->db->from('order_items');
        $this->db->join('products', 'products.id = order_items.product_id', 'left');
        $this->db->join('variations', 'variations.id = order_items.variation_id', 'left');
